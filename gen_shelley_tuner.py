#!/usr/bin/env python3
"""SHELLEY SOLO TUNER (7-27) — single-tile clone of the 200-col command
center, made because her saturated-green-wall render was tonally INVERTED
(the wd wall-distance tone map fills bright skin and empties mid-dark
features). New knobs vs the main CC:
- "Tone mapping" segment: Standard ink (d=1-brv, how everyone else renders —
  the page DEFAULT) vs Wall-distance (|brv-0.5|*2, Ruben's mapping).
  Engine field: `wdTone` ("std"/"wall"), only meaningful on wd clips.
- "Mask reach" slider = `wdDist` (chromaticity cutoff of the saturated-wall
  mask, default 0.12).
BASE starts from Marco's 2026-07-27 finals (global + his Shelley overrides)
except gamma 2.7 (his 6.85 was tuned against the wall-distance map).
Own localStorage key (lfShelleyCC1); Save final writes
lf-shelley-final-*.json — merge it back into the main finals as Shelley's
per-video overrides. Same master-clock playback as the CC."""
import json
import pathlib
import subprocess
...
OUT = pathlib.Path(__file__).parent / "lf-shelley-tuner.html"
...
# Marco's 2026-07-27 finals (global + his Shelley per-video overrides),
# except: wdTone "std" (the fix under test) and gamma 2.7 (his 6.85 was
# tuned against the wall-distance tone map this page defaults away from).
BASE = {
    "cols": 172, "ramp": "PIXLF", "dir": "ink", "gamma": 2.7, "floor": 0.0,
    ...
    "pxc": "auto", "loopLen": 3.9, "start": 0, "end": END_AUTO,
    "wdTone": "std", "wdDist": 0.12,
}

HTML = r"""<title>Shelley — Solo Tuner</title>
<style>
__FONTS__
...
<header>
  <h1><span class="dot"></span>Shelley &middot; Solo Tuner<small>fix her tone to match the others &mdash; then Save final and hand the file to Claude</small></h1>
...
GROUPS.forEach(g=>{
  ...
  if(g.title==="Pixel grid"){
    ...
    // the inversion fix: Standard ink = how everyone else renders (dark
    // features dense, bright skin light); Wall-distance = Ruben's mapping
    sec.appendChild(segEl("Tone mapping", [{label:"Standard ink",value:"std"},{label:"Wall-distance",value:"wall"}], "wdTone", ()=>effSel().wdTone||"std"));
...
(function(){
  ...
  b.addEventListener("click", ()=>{
    ...
    window.claude.downloads.save({filename:"lf-shelley-final-"+stamp+".json", data:payload})
...
// ---- persistence ----
function save(){ try{ localStorage.setItem("lfShelleyCC1", JSON.stringify({G,O})); }catch(e){} }
(function restore(){
  try{
    const st = JSON.parse(localStorage.getItem("lfShelleyCC1")||"null");
