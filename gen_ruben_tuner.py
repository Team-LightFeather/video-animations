#!/usr/bin/env python3
"""RUBEN SOLO TUNER (7-27) — single-tile clone of the 200-col command
center (sibling of gen_shelley_tuner.py) so Marco can dial Ruben's
gray-wall/white-shirt render in isolation. Differences vs the main CC:
- Contrast (gamma) range extended past 8: 0.4–16 (his finals sat at 7.2,
  pinned against the old max).
- "Tone mapping" segment: Wall-distance (|brv-0.5|*2 — Ruben's current
  mapping, the page DEFAULT) vs Standard ink (d=1-brv, everyone else).
- "Mask reach" slider = `wdDist` (only affects saturated-wall clips; inert
  on Ruben's neutral gray wall, exposed for completeness).
BASE = Marco's 2026-07-27 finals (global + his Ruben overrides: gamma 7.2,
cols 174, bright 0.5, window 1.1–4.7s). Own localStorage key (lfRubenCC1);
Save final writes lf-ruben-final-*.json — merge it back into the main
finals as Ruben's per-video overrides. Same master-clock playback."""
import json
import pathlib
import subprocess
...
OUT = pathlib.Path(__file__).parent / "lf-ruben-tuner.html"
...
# Marco's 2026-07-27 finals: global + his Ruben per-video overrides
# (gamma 7.2, cols 174, bright 0.5, window 1.1-4.7s). wdTone "wall" = his
# current mapping (white shirt reads dense on the gray wall).
BASE = {
    "cols": 174, "ramp": "PIXLF", "dir": "ink", "gamma": 7.2, "floor": 0.0,
    "minOp": 0.10, "bright": 0.5, "color": "mono", "scan": 0, "edge": 0.15,
    "gScale": 1.28, "weight": 700, "stab": 0, "smooth": 0, "bands": 0,
    "blur": 0, "lfThr": 0.75, "pixFill": 0.66, "lfEdge": 0, "lfFill": "blocks",
    "pxc": "auto", "loopLen": 3.9, "start": 1.1, "end": 4.7,
    "wdTone": "wall", "wdDist": 0.12,
}

HTML = r"""<title>Ruben — Solo Tuner</title>
<style>
__FONTS__
...
#mosaic{display:grid;grid-template-columns:1fr;grid-template-rows:1fr;
  gap:0;width:100%;max-width:calc((100vh - 150px)*432/454);
...
<header>
  <h1><span class="dot"></span>Ruben &middot; Solo Tuner<small>contrast unlocked to 16 &mdash; then Save final and hand the file to Claude</small></h1>
...
const GROUPS = [
  ...
  {title:"Tone", knobs:[
    {f:"gamma",  label:"Contrast",    min:0.4, max:16, step:0.05, fmt:v=>v.toFixed(2)},
    ...
    {f:"wdDist", label:"Mask reach",  min:0.05, max:0.30, step:0.01, fmt:v=>v.toFixed(2)},
  ]},
  {title:"Motion", knobs:[
...
GROUPS.forEach(g=>{
  ...
  if(g.title==="Pixel grid"){
    ...
    // Wall-distance = Ruben's current mapping (white shirt reads dense on
    // the gray wall); Standard ink = how the light-wall clips render
    sec.appendChild(segEl("Tone mapping", [{label:"Wall-distance",value:"wall"},{label:"Standard ink",value:"std"}], "wdTone", ()=>effSel().wdTone||"wall"));
...
(function(){
  ...
  b.addEventListener("click", ()=>{
    ...
    window.claude.downloads.save({filename:"lf-ruben-final-"+stamp+".json", data:payload})
...
// ---- persistence ----
function save(){ try{ localStorage.setItem("lfRubenCC1", JSON.stringify({G,O})); }catch(e){} }
(function restore(){
  try{
    const st = JSON.parse(localStorage.getItem("lfRubenCC1")||"null");
