...
OUT = pathlib.Path(__file__).parent / "lf-people-inpage-preview-120.html"
...
# 120-COL VARIANT (Marco 7-27): every video renders at 120 cols — the
# global is pinned and per-person cols overrides are stripped; everything
# else (gamma/tone/playback windows) stays from the merged finals.
BASE = dict(FINALS["settings"]["global"])
BASE["cols"] = 120
O = {name: {k: v for k, v in ovr.items() if k != "cols"}
     for name, ovr in FINALS["settings"]["perVideo"].items()}
O = {name: ovr for name, ovr in O.items() if ovr}

HTML = r"""<title>LF People — In-Page Preview · 120 cols</title>
<style>
__FONTS__
